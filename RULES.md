...
2. **처음부터 제물/하수인으로 게임을 시작한 딱 2명**(`generateRelicLayout`이
   `["sacrifice", "minion"]`을 직접 배정)은 "그 전에 들고 있던 진짜 유물"이라는 게
   아예 없다 — 그래서 여기 안 걸리고, `refreshFakeBeliefs`가 처음 접근할 때
   무작위 풀(`villagerPool`/`minionPool`)에서 골라준다.
